...
import {Box, Button, Stack, Center, Heading} from '@chakra-ui/react'
    ...
    <>
        <Box w='100%' h={'auto'} p={10} color='white' bg={theme ? 'white' : 'gray.700'}>
            ...
            <Center bg={theme? 'blue.50' : 'gray.800'}>
                <Heading fontFamily={'sans-serif'} fontSize={{base: '3xl', md: '5xl'}} p={2} color={theme? 'blue.600' : 'linkedin.500'}>Lista completa</Heading>
            </Center>
            ...
            <Box display={'flex'} flexFlow={'wrap'} justifyContent={'center'} overflowY={'auto'} >
                {Array.map(e=> <MoviesContainer votos={e.votos} key={e.id} img={e.img} title={e.title} rating={e.rating} reviewCount={e.reviewCount} id={e.id}  />)}
            </Box>
